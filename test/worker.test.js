...
test("domains page shows dedicated link form and latest messages panel", async () => {
  ...
  assert.match(html, /邮箱生成器/);
  assert.match(html, /class="tool-panel generator-panel"/);
  assert.match(html, /id="emailGenerator"/);
  assert.match(html, /id="generatorDomain"/);
  assert.match(html, /id="generatedEmail"/);
  assert.match(html, /生成邮箱/);
  assert.match(html, /创建专属链接/);
  assert.match(html, /class="tool-panel link-form"/);
  assert.match(html, /name="address"/);
  assert.match(html, /action="\/admin\/link"/);
  assert.match(html, /id="linkResult"/);
  assert.match(html, /专属链接/);
  assert.doesNotMatch(html, /id="copyAddress"/);
